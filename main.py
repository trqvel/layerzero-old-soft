...
from modules.exhange_buy_native import okx_main
...
def path_processing(private_key, file_path):
    ...
    for i, el in enumerate(paths):
        ...
        if status is None or status != "Success":
            if status == "Pending":
                if "txn_hash" in el and get_transaction_status(el["hash"], private_key):
            ...
            if el['type'] == 'Bridge':
                ...
                if el['project'] == 'Harmony':
                    amount = el['amount']
                    result = harmony_bridge(private_key, amount, from_chain, from_token, paths, file_path, i)
                if el['project'] == 'Stargate':
                    result = stargate_bridge(private_key, from_chain, to_chain, from_token, to_token, paths, file_path, i)
                if el['project'] == 'BTCb':
                    result = btcb_bridge(private_key, from_chain, to_chain, from_token, paths, file_path, i)
                if el['project'] == 'Testnet':
                    amount = el['amount']
                    result = testnet_bridge(private_key, amount, from_chain, "ETH", paths, file_path, i)
                if el['project'] == 'Aptos':
                    amount = el['amount']
                    aptos_address = el['address']
                    result = aptos_bridge(private_key, amount, from_chain, from_token, aptos_address, paths, file_path, i)
            if el['type'] == 'Buy':
                delay(delay_txn)
                from_chain = el['from']
                from_token = el['token'][0]
                to_token = el['to_token'][0]
                amount = el['amount']
                result = choose_and_call_swap(private_key, from_token, from_chain, amount, to_token, paths, file_path, i)
            if el['type'] == 'Sell':
                delay(delay_txn)
                from_chain = el['from']
                from_token = el['token'][0]
                to_token = el['to_token'][0]
                amount = el['amount']
                result = woofi_swap_sell_btcb(private_key, from_token, from_chain, amount, to_token, paths, file_path, i)
            if el['type'] == 'Staking':
                delay(delay_txn)
                from_chain = el['from']
                from_token = el['token'][0]
                ...
                if not stake_status:
                    print_with_time("  Токен STG уже застейкан. Пропускаем стейкинг ...")
                    print(f'          {get_address_wallet(private_key)}')
                if stake_status:
                    result = stake_stg(private_key, from_chain, from_token, paths, file_path, i)

        if result == False and status != "Success":
            print_with_time(
                "  Произошла ошибка")
...
def ask_overwrite(wallets_list, wallet_address_map, aptos_address_map):
    ...
    if existing_paths > 0:
        print(f"          Сгенерировать новые пути для аккаунтов? (1/0)")
        answer = int(input(f"          "))
        if answer == 1:
    ...
    if generated_paths_count > 0:
        ...
        print_with_time(f"  Сгенерировано {generated_paths_count} путей. Начинаем работать? (1/0):")
        proceed = int(input(f"          "))
        if proceed == 1:
            return True
        else:
...
def main():
    ...
    if not proceed:
        print("Завершаем работу скрипта, так как ответ не содержал: 1...\n")
        return
